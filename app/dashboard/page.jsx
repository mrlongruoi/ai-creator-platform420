              ...
              {!postsLoading && recentPosts && recentPosts.length > 0 && (
                <div className="space-y-4">
                  {recentPosts.map((post) => {
                    const editHref = `/dashboard/posts/edit/${post._id}`;
                    let badgeVariant;
                    let badgeClass;
                    ...
                    if (post.status === "published" && post.publishedAt) {
                      statusText = `Published ${formatTime(post.publishedAt)}`;
                    } else if (post.status === "draft") {
                      statusText = `Updated ${formatTime(post.updatedAt)}`;
                    } else if (post.scheduledFor) {
                      statusText = `Scheduled for ${new Date(post.scheduledFor).toLocaleDateString()}`;
                    } else {
                      statusText = `Updated ${formatTime(post.updatedAt)}`;
                    }
                    return (
                      <Link
                        key={post._id}
                        href={editHref}
                        className="flex items-center justify-between p-4 bg-slate-800/30 hover:bg-slate-700/30 rounded-lg transition-colors"
                      >
              ...
              {!activityLoading && recentActivity && recentActivity.length > 0 && (
                <div className="space-y-4">
                  {recentActivity.map((activity) => {
                    const key = activity._id ?? `${activity.type}-${activity.time}`;
                    let colorClass;
                    ...
                    return (
                      <div key={key} className="flex items-start gap-3">
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center text-xs ${colorClass}`}
                        >
                        ...
                        <div className="flex-1">
                          <p className="text-sm text-white">
                            ...
                            {activity.type === "comment" && ` commented on "${activity.post}"`}
